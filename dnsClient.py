...
def dns_query(type, name, server):
    ...
    server_address = (server, 53)  # Port 53 for DNS
    ...
    # Encode the QNAME
    qname_parts = name.split('.')  # Split the domain name by '.'
    ...
    data, _ = sock.recvfrom(4096)  # 4096 Bytes buffer size
